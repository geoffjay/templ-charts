...
**not** in v4. It is captured in [`docs/plans/tracking.md`](tracking.md) as a
...
[`docs/plans/v1.md`](v1.md), [`docs/plans/v2.md`](v2.md), and
[`docs/plans/v3.md`](v3.md) for the prior structure this document mirrors.
...
| Convenience API | **Render helpers only** — a small `charts/render` package (`String`/`To`). **No** public sample-data export and **no** extension of the `charts/static` registry beyond its v1 bar/line/pie scope (both deferred — see [`tracking.md`](tracking.md)). |
...
| Deferred | Consolidate all v1/v2/v3 §11 deferrals + everything else the audit surfaced into [`docs/plans/tracking.md`](tracking.md). |
...
- **Animation is v1-only.** `MotionProps{Animate:true}` is defaulted on ≥8 v2/v3
  ...
  no-ops). Full v2/v3 animation parity → [`tracking.md`](tracking.md).
...
- **Load/showcase demo page** (`/benchmark` in the demo app): renders a chart at
  ...
  [`tracking.md`](tracking.md)) a baseline to beat.
...
**Explicitly out of scope** (→ [`tracking.md`](tracking.md)): Canvas
...
in [`docs/plans/tracking.md`](tracking.md), the consolidated backlog that
